declare name "tsarWeeded"; // courtesy of Romain Michon's STK Modal Bar
...
biquadBankX = _ <: sum(i, keep, oneFilter(i+skip)) 
  with{
    oneFilter(j,exc) =  loadCF(j), dampTilt(loadCF(j)), excTilt(loadCF(j))*exc*(pow(loadX(j),whackBoost)): bandPassH; 
  };
biquadBankY = _ <: sum(i, keep, oneFilter(i+skip)) 
  with{
    oneFilter(j,exc) =  loadCF(j), dampTilt(loadCF(j)), excTilt(loadCF(j))*exc*(pow(loadY(j),whackBoost)): bandPassH; 
  };
biquadBankZ = _ <: sum(i, keep, oneFilter(i+skip)) 
  with{
    oneFilter(j,exc) =  loadCF(j), dampTilt(loadCF(j)), excTilt(loadCF(j))*exc*(pow(loadZ(j),whackBoost)): bandPassH; 
  };
